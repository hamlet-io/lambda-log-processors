...
class Ingester:
    def __init__(self, sinks, bucket_name, key):
        self.sinks = sinks
    ...
    def run(self):
        ...
        print("Got %s message%s" % (len(msgs), "" if 1 == len(msgs) else "s"))
    ...
    def from_environment(cls):
        sinks = []
        if "ELASTICSEARCH_API_FQDN" in os.environ and len(
            os.environ.get("ELASTICSEARCH_API_FQDN")
        ):  # NOQA
            use_sig4 = (
                os.environ.get("ELASTICSEARCH_API_AUTH")
                .casefold()
                .startswith("sig4".casefold())
            )  # NOQA
